...
impl ConcatKDF {
    ...
    pub(super) fn kdf(
        ...
        let party_v_info = Self::prepend_len_base64(party_v_info)?;
        let supp_pub_info = (key_data_len_bits as u32).to_be_bytes();

        Ok(self.kdf_internal(
            shared_secret,
            key_data_len_bits,
            &algorithm_id,
            &party_u_info,
    ...
    /// Length-prefixes the base64url-DECODED octets of an `apu`/`apv` value.
    /// An empty value yields the four-byte zero length (no data), matching
    /// jose4j's `KdfUtil.getDatalenDataFormat`.
    fn prepend_len_base64(encoded: &str) -> Result<Box<[u8]>, JoseError> {
        if encoded.is_empty() {
            return Ok(Box::new([0, 0, 0, 0]));
        }
        let decoded = base64::url_decode(encoded)?;
        Ok(Self::prepend_len(decoded))
    }
}
...
mod tests {
    ...
    fn test_concat_kdf1() {
        ...
        let derived_key = kdf
            .kdf(&shared_secret, key_data_len_bits, algorithm_id, "", "")
            .unwrap();
    ...
    fn test_concat_kdf2() {
        ...
        // base64url("Alice") and base64url("Bob") -- apu/apv are decoded pre-KDF.
        let party_u_info = "QWxpY2U";
        let party_v_info = "Qm9i";
