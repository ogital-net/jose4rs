...
impl AesHmacAeadCtx {
    ...
    pub(super) fn encrypt<'a>(
        ...
        // M are used as T.
        let mut mac_buf = [0u8; crate::crypto::MAX_DIGEST_LEN];
        let m = mac(self.algorithm, mac_key, aad, iv, e, &al, &mut mac_buf)?;
...
/// Computes the AES-CBC-HMAC authentication tag input HMAC over the disjoint
/// slices `aad || iv || ciphertext || al` without concatenating them.
fn mac<'a>(
    algorithm: Algorithm,
    mac_key: &[u8],
    aad: &[u8],
    ...
    ciphertext: &[u8],
    al: &[u8],
    buf: &'a mut [u8; crate::crypto::MAX_DIGEST_LEN],
) -> Result<&'a [u8], JoseError> {
    let mut ctx = hmac::HmacCtx::init(algorithm.digest_algorithm(), mac_key)?;
    ctx.update(aad);
    ctx.update(iv);
    ctx.update(ciphertext);
    ctx.update(al);
    Ok(ctx.finish_buf(buf))
}
