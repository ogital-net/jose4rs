...
pub struct JwtConsumerBuilder {
    ...
    max_future_validity_minutes: Option<i64>,
...
impl JwtConsumerBuilder {
    ...
    pub fn set_allowed_clock_skew_in_seconds(mut self, seconds: i64) -> Self {
        self.allowed_clock_skew_secs = seconds;
        self
    }

    /// Set the maximum on how far in the future the exp claim can be
    pub fn set_max_future_validity_in_minutes(mut self, minutes: i64) -> Self {
        self.max_future_validity_minutes = Some(minutes);
        self
    }
    ...
    pub fn build(self) -> JwtConsumer {
        JwtConsumer {
            ...
            allowed_clock_skew_secs: self.allowed_clock_skew_secs,
            max_future_validity_minutes: self.max_future_validity_minutes,
...
impl JwtConsumer {
    ...
    pub fn process_to_claims(&self, claims: &JwtClaims) -> Result<(), InvalidJwtError> {
        ...
        if let Some(expected_audiences) = &self.expected_audiences {
            if let Some(audience) = audience {
                ...
                if !expected_audiences.is_empty() {
                    let has_match = audience
                        .iter()
                        .any(|aud| expected_audiences.contains(aud));
                    if !has_match {
                        return Err(InvalidJwtError::with_error_code(
        ...
        if let Some(expected_subject) = &self.expected_subject {
            if let Some(subject) = subject {
                if subject != expected_subject {
                    return Err(InvalidJwtError::with_error_code(
                        format!("Subject '{}' does not match expected '{}'", subject, expected_subject),
    ...
    fn validate_time_claims(&self, claims: &JwtClaims) -> Result<(), InvalidJwtError> {
        ...
        let eval_secs = eval_time.duration_since(UNIX_EPOCH).unwrap().as_secs() as i64;

        let mut errors = Vec::new();

        // Validate expiration time
        if let Some(exp) = claims.get_expiration_time() {
            let exp_secs = exp.duration_since(UNIX_EPOCH).unwrap().as_secs() as i64;

            // Check if expired (token is expired at or after the exp time)
            if exp_secs + self.allowed_clock_skew_secs <= eval_secs {
                errors.push(ErrorCode::Expired);
            }

            // Check if expiration is too far in the future
            if let Some(max_future_validity) = self.max_future_validity_minutes {
                let max_exp = eval_secs + (max_future_validity * 60);
                if exp_secs > max_exp + self.allowed_clock_skew_secs {
        ...
        if let Some(nbf) = claims.get_not_before() {
            let nbf_secs = nbf.duration_since(UNIX_EPOCH).unwrap().as_secs() as i64;

            if nbf_secs > eval_secs + self.allowed_clock_skew_secs {
        ...
        if let Some(iat) = claims.get_issued_at() {
            ...
            if let Some(allowed_future) = self.iat_allowed_secs_in_future {
                if iat_secs > eval_secs + allowed_future {
            ...
            if let Some(allowed_past) = self.iat_allowed_secs_in_past {
                if iat_secs < eval_secs - allowed_past {
...
mod tests {
    ...
    fn test_max_future_validity() {
        ...
        let consumer = JwtConsumerBuilder::new()
            .set_evaluation_time_from_seconds(1000)
            .set_max_future_validity_in_minutes(10)
            .build();
