...
mod jwt_consumer_tests {
    ...
    fn some_basic_aud_checks() {
        ...
        let claims = JwtClaims::parse(r#"{"aud":"example.com"}"#).unwrap();
        ...
        let claims = JwtClaims::parse(r#"{"sub":"subject"}"#).unwrap();
        let consumer = JwtConsumerBuilder::new()
        ...
        let claims = JwtClaims::parse(r#"{"aud":["example.com","usa.org","ca.ca"]}"#).unwrap();
        let consumer = JwtConsumerBuilder::new()
            .set_expected_audience(true, false, &["example.org"])
            .build();
        assert!(consumer.process_to_claims(&claims).is_err());
    ...
    fn some_basic_iss_checks() {
        let claims = JwtClaims::parse(r#"{"iss":"issuer.example.com"}"#).unwrap();
        ...
        let claims = JwtClaims::parse(r#"{"iss":"accounts.google.com"}"#).unwrap();
        let consumer = JwtConsumerBuilder::new()
            .set_expected_issuers(true, &["https://accounts.google.com", "accounts.google.com"])
            .build();
        assert!(consumer.process_to_claims(&claims).is_ok());

        let claims = JwtClaims::parse(r#"{"iss":"https://accounts.google.com"}"#).unwrap();
        assert!(consumer.process_to_claims(&claims).is_ok());
    ...
    fn some_basic_sub_checks() {
        let claims = JwtClaims::parse(r#"{"sub":"brian.d.campbell"}"#).unwrap();
        ...
        // Missing subject
        let claims = JwtClaims::parse(r#"{"name":"brian.d.campbell"}"#).unwrap();
        let consumer = JwtConsumerBuilder::new()
            .set_require_subject()
            .build();
        assert!(consumer.process_to_claims(&claims).is_err());
    ...
    fn some_basic_jti_checks() {
        let claims = JwtClaims::parse(r#"{"jti":"1Y5iLSQfNgcSGt0A4is29"}"#).unwrap();
        ...
        // Missing JTI
        let claims = JwtClaims::parse(r#"{"notjti":"lbZ_mLS6w3xBSlvW6ULmkV-uLCk"}"#).unwrap();
        let consumer = JwtConsumerBuilder::new()
            .set_require_jwt_id()
            .build();
        assert!(consumer.process_to_claims(&claims).is_err());
    ...
    fn some_basic_time_checks() {
        ...
        let claims = JwtClaims::parse(r#"{"sub":"brian.d.campbell","exp":1430602000}"#).unwrap();
        let consumer = JwtConsumerBuilder::new()
            .set_require_expiration_time()
        ...
        let consumer = JwtConsumerBuilder::new()
            .set_require_expiration_time()
            .set_evaluation_time_from_seconds(1430602000)
            .set_allowed_clock_skew_in_seconds(10)
            .build();
        assert!(consumer.process_to_claims(&claims).is_ok());
        ...
        // Test nbf (not before)
        let claims = JwtClaims::parse(r#"{"sub":"brian.d.campbell","nbf":1430602000}"#).unwrap();
        let consumer = JwtConsumerBuilder::new()
            .set_evaluation_time_from_seconds(1430602000)
            .build();
        assert!(consumer.process_to_claims(&claims).is_ok());
        ...
        let consumer = JwtConsumerBuilder::new()
            ...
            .set_allowed_clock_skew_in_seconds(30)
            .build();
        assert!(consumer.process_to_claims(&claims).is_ok());
        ...
        let claims = JwtClaims::parse(r#"{"sub":"brian.d.campbell","exp":1430607201}"#).unwrap();
        let consumer = JwtConsumerBuilder::new()
            .set_require_expiration_time()
        ...
        let consumer = JwtConsumerBuilder::new()
            .set_require_expiration_time()
            .set_evaluation_time_from_seconds(1430600000)
            .set_max_future_validity_in_minutes(90)
            .build();
        assert!(consumer.process_to_claims(&claims).is_err());
        ...
        let consumer = JwtConsumerBuilder::new()
            .set_require_expiration_time()
            .set_evaluation_time_from_seconds(1430600000)
            .set_max_future_validity_in_minutes(120)
            .build();
        assert!(consumer.process_to_claims(&claims).is_err());
        ...
        let consumer = JwtConsumerBuilder::new()
            ...
            .set_evaluation_time_from_seconds(1430600000)
            .set_max_future_validity_in_minutes(120)
            .set_allowed_clock_skew_in_seconds(20)
            .build();
        assert!(consumer.process_to_claims(&claims).is_ok());
    }
    ...
    fn iat_reasonableness() {
        ...
        let claims = JwtClaims::parse(r#"{"iat":1571322100}"#).unwrap();
        assert!(consumer.process_to_claims(&claims).is_ok());

        // Valid iat 1 second in past
        let claims = JwtClaims::parse(r#"{"iat":1571322099}"#).unwrap();
        assert!(consumer.process_to_claims(&claims).is_ok());

        // Valid iat within allowed past
        let claims = JwtClaims::parse(r#"{"iat":1571322043}"#).unwrap();
        assert!(consumer.process_to_claims(&claims).is_ok());

        let claims = JwtClaims::parse(r#"{"iat":1571322040}"#).unwrap();
        assert!(consumer.process_to_claims(&claims).is_ok());

        // Too far in past
        let claims = JwtClaims::parse(r#"{"iat":1571322039}"#).unwrap();
        assert!(consumer.process_to_claims(&claims).is_err());

        let claims = JwtClaims::parse(r#"{"iat":1570321001}"#).unwrap();
        assert!(consumer.process_to_claims(&claims).is_err());

        // Too far in future
        let claims = JwtClaims::parse(r#"{"iat":1571322101}"#).unwrap();
        assert!(consumer.process_to_claims(&claims).is_err());

        let claims = JwtClaims::parse(r#"{"iat":1571322177}"#).unwrap();
        assert!(consumer.process_to_claims(&claims).is_err());
        ...
        // 120 seconds in past
        let claims = JwtClaims::parse(r#"{"iat":1571321980}"#).unwrap();
        assert!(consumer.process_to_claims(&claims).is_ok());

        // 121 seconds in past
        let claims = JwtClaims::parse(r#"{"iat":1571321979}"#).unwrap();
        assert!(consumer.process_to_claims(&claims).is_err());

        // 5 seconds in future
        let claims = JwtClaims::parse(r#"{"iat":1571322105}"#).unwrap();
        assert!(consumer.process_to_claims(&claims).is_ok());

        // 10 seconds in future
        let claims = JwtClaims::parse(r#"{"iat":1571322110}"#).unwrap();
        assert!(consumer.process_to_claims(&claims).is_ok());

        // 11 seconds in future
        let claims = JwtClaims::parse(r#"{"iat":1571322111}"#).unwrap();
        assert!(consumer.process_to_claims(&claims).is_err());
    }
    ...
    fn some_basic_checks() {
        let claims =
            JwtClaims::parse(r#"{"sub":"subject","iss":"issuer","aud":"audience"}"#).unwrap();
    ...
    fn error_code_validation() {
        ...
        let consumer = JwtConsumerBuilder::new()
            .set_expected_audience(true, false, &["AUD"])
            .set_expected_issuer("ISS")
            .set_max_future_validity_in_minutes(5)
            .set_require_expiration_time()
            .build();
