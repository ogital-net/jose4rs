...
Early release (0.6.x): the API may still change between minor versions, but the implementation is complete for the standards listed below and is covered by an extensive test suite, including the jose4j test vectors.
...
```toml
[dependencies]
jose4rs = { version = "0.6", features = ["zip"] }
